...
from utils_face.utils import (Alignment_1, compare_faces, letterbox_image,
                              preprocess_input)
from utils_face.utils_bbox import (decode, decode_landm, non_max_suppression,
...
def cv2ImgAddText(img, label, left, top, textColor=(255, 255, 255)):
    ...
    #   设置字体
    # ---------------#
    font = ImageFont.truetype(font='model_data_face/simhei.ttf', size=20)
...
class Retinaface(object):
    _defaults = {
        ...
        #   retinaface训练完的权值路径
        # ----------------------------------------------------------------------#
        "retinaface_model_path": 'model_data_face/Retinaface_mobilenet0.25.pth',
        # ----------------------------------------------------------------------#
        ...
        #   facenet训练完的权值路径
        # ----------------------------------------------------------------------#
        "facenet_model_path": 'model_data_face/facenet_mobilenet.pth',
        # ----------------------------------------------------------------------#
    ...
    def __init__(self, encoding=0, **kwargs):
        ...
        try:
            self.known_face_encodings = np.load(
                "model_data_face/{backbone}_face_encoding.npy".format(backbone=self.facenet_backbone))
            self.known_face_names = np.load(
                "model_data_face/{backbone}_names.npy".format(backbone=self.facenet_backbone))
        except:
            if not encoding:
    ...
    def encode_face_dataset(self, image_paths, names ,DeviceName):
        face_encodings = []
    ...
    def detect_image(self, image):
        ...
        for i, b in enumerate(boxes_conf_landms):
            ...
            #   如果不是必须，可以换成cv2只显示英文。
            # --------------------------------------------------------------#
            old_image = cv2ImgAddText(old_image, name, b[0] + 5, b[3] - 25, (255, 255, 0))
        if len(boxes_conf_landms) > 1:
            return old_image, 2
        return old_image, self.flag
