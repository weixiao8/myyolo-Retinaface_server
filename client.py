...
async def send_msg(websocket):
    deviceid = "NJXQLYFZYXGS-002"
    await websocket.send(deviceid)
    ...
    if filename == "404":
        ...
        head2 = await websocket.recv()
        filename = struct.unpack(fmt, head2)[0].decode().rstrip('\0')
        print(f"{head2}")
        print(f"{filename}")
        filename = './model_data_face/' + filename
        print(f"{filename}")
...
async def main_logic():
    async with websockets.connect('ws://127.0.0.1:2022') as websocket:
